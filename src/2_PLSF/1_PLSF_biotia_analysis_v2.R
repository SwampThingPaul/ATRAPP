...
# test=subset(plsf.phyto,CY==2016&month==7);test
# ddply(test,c("class"),summarise,mean.biovol.um3mL=sum(totbiovol.um3mL,na.rm=T),
#       mean.conc.cellsmL=sum(Conc.cellsmL,na.rm=T))
# sum(test$Conc.cellsmL)
# sum(test$totbiovol.um3mL)
...
## Barry's Spreadsheet with dizatroph and non-diazatroph
# tmp=read.csv(paste0(data.path,"PLSF_Microscope/SppList/20230317_GenusSpp_ClassOrder-BH.csv"))
# dput(unique(subset(tmp,Diazotroph.1==1)$genus))
# dput(unique(subset(tmp,Diazotroph.1==2)$genus))
# 
# chk.gen=ddply(tmp,c("genus"),summarise,N.diaz=sum(Diazotroph.1==1,na.rm=T),N.nondiaz=sum(Diazotroph.1==2,na.rm=T),N.val=N.obs(genus))
# chk.gen[order(-chk.gen$N.diaz),]
# chk.gen[order(-chk.gen$N.nondiaz),]
# chk.class=ddply(tmp,c("class"),summarise,N.diaz=sum(Diazotroph.1==1,na.rm=T),N.nondiaz=sum(Diazotroph.1==2,na.rm=T),N.val=N.obs(class))
...
# spp.class.order$nondiaztroph=with(spp.class.order,ifelse(genus%in%nondiazotroph.genera|
#                                                            GenusSpp%in%other.nondiaz.GenusSpp,1,NA))
#                                                         
# chk2=ddply(spp.class.order,c("genus"),summarise,
#           N.diaz=sum(diaztroph==1,na.rm=T),
#           N.nondiaz=sum(nondiaztroph==1,na.rm=T),
#           N.val=N.obs(genus))
# # head(chk.gen[order(-chk.gen$N.diaz),],16)
# head(chk2[order(-chk2$N.diaz),],16); # vary slightly because I fixed the mmaster spp list
# # head(chk.gen[order(-chk.gen$N.nondiaz),],16)
# head(chk2[order(-chk2$N.nondiaz),],16)
